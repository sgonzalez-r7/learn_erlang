...
queue_card_fields(Email) ->
  [reported_email_from(Email),
   reported_email_subject(Email),
   submitter(Email),
   reported_email_date(Email)].
...
submitter(Email) ->
  {_T, _ST, H, _P, _B} = Email,
  {<<"From">>, V} = proplists:lookup(<<"From">>, H),
  {<<"Submitter">>, V}.
...
header(Key, Email) ->
  {_T, _ST, H, _P, _B} = attached_email(Email),
  proplists:lookup(Key, H).

attached_email(Email) ->
  {_T, _ST, _H, _P, B} = Email,
  case B of
    Parts when is_list(Parts) ->
      {_, _, _, _, Attached} = find_part({<<"message">>, <<"rfc822">>}, Parts),
      Attached;
    _Else ->
      none
  end.
...
find_part({Type, SubType}, Parts) ->
  Hit = fun({T, ST, _H, _P, _B}) -> T =:= Type andalso ST =:= SubType end,
  hd(lists:filter(Hit, Parts)).
