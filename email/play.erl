...
parse({Type, Sub_Type, Headers, Parameters, Body}) ->
  case Body of
...
parse_headers(Headers) ->
  Normalized = lists:map(fun(X) -> normalize_header_key(X) end, Headers),
  maps:from_list(Normalized).

normalize_header_key({K, V}) ->
  To_Underscore = binary:replace(K, <<"-">>, <<"_">>, [global]),
  {binary_to_lower_atom(To_Underscore), V}.


binary_to_lower_atom(Binary_String) ->
  list_to_atom(string:to_lower(binary_to_list(Binary_String))).
