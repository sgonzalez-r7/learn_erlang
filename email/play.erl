...
%base case
parse(Body) when is_binary(Body) -> Body;

parse(Body) when is_list(Body) ->
  lists:map(fun(B) ->
    parse(B)
  end, Body);
...
parse({T, ST, Hs, Ps, B}) ->
  #{
    <<"type">> => binary:list_to_bin(binary:bin_to_list(T) ++
                  "_" ++
                  binary:bin_to_list(ST)),
    <<"headers">> => normalize(Hs) ++ normalize(Ps),
    <<"body">> => parse(B)
  }.

normalize(Props_List) ->
  lists:map(fun({K, V}) ->
    Snakeified = binary:replace(K, <<"-">>, <<"_">>, [global]),
    Stringified = binary_to_list(Snakeified),
    Downcased = string:to_lower(Stringified),
    Normalized = list_to_binary(Downcased),
    {Normalized, V}
  end, Props_List).
